...
How would watching end ?

How to efficiently walk the whole tree of a repo, but also walk all the branches too ?
What is the data structure that can provide all these views and also provide streaming updates when anything changed ?
...
These should be pulled down as a GET from the server, with the commit that was given in the splice.  Allows us to give access to binary files without choking SSE for it.  There is no diffing, so more efficient.
If the full git repo was replicated on client side, then we could just return from the local oid first ?

...
**Derive** an environment repo ID in a stable, predictable way from the DID itself. For example, `repoId = sha256('env:' + userDID)`.

only allow the superuser did to create additional user dids in the system.
When db is blank, we run a provisioning tool on it to make a new repo in a safe concurrency safe way.

Registry that points repos to parents, so we can walk up and down the trees. atomic guarantees we never have orphans.

So store the links, both ways, in kv under:
links/down/repoId -> repoChild
links/up/repoChild -> repoId

This allows bidirectional walking.
...
We can issue NFTs on opensea along with conditions that prohibit the trading of them as per securities laws.  Then if you trade the NFT the DRE object can know and will blacklist the backing NFT as illegal security.  You can re-whitelist it if you are the owner of the addresses that it was transferred to.  Status of the item can be referred to by a mutable URL on opensea, so we can infer the current securities status. 

The crime is with the seller, not us, if we took steps to block illegal sales.

One off funding sales for legitimate reasons are completely ok and are a good way for us to get funds in without payment processing fees.
...
Age of offer
Chance of retraction based on gas history
Basically a plugin system so people can add on tools easily and get paid based on how much they get used - blockchain folk will understand the per usage charging model.
Know when fake offers are present - when there is not enough weth to fulfill an offer, we know they are some kind of scammer.
Know their acquisition cost, so know when they are listing for less than they paid, which means it is likely a better deal.
Score pressure to sell - show the price that has been on offer for the past x months to work out pressure that was there to sell, so know that if nobody sold in that time, then the item is tightly held.
Profile history - watch behaviour of traders and score them as degen or otherwise, bot assisted, new.

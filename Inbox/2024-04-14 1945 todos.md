...
make an anonymous session chain as the first chain to connect to, pre auth
trace every single commit reliably
session management
use sessions properly in the ui
github login
run a full chat session with chat as a side effect 
test replay of side effects
show threads of help invocations in UI
backstage admin view for graphs of the whole system - show historical token rate
write to the session file constantly - so switch isolates to be staged

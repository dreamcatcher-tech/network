...
show number of sessions that were created
also show number of positive sentiments from paying people
agent processing in the crm - start a long running process and then allow the user to carry on with something else
private chat by default - detect private info in chat - mention to the user when private
updating a file on the filesystem
agent crawlers that go off and do things but let HAL keep chatting.
advertise the voice input function, and annotate transcriptions for HAL.
HAL emotional feedback based on how he is being treated with a color and a facial expression using a 3d face generator
fork a session
mark a session as readonly when shared so changes need to be forked
be able to see what forks have been made of one of your sessions
Fork at any point in the session using hotkeys or backchat
be able to make lists of user sessions and refer to them in links, so can work on users data.
transform a session into a pseudosession
Detect context switch and archive off previous sessions, but allow them to get it back using a backchat function
backchat functions without asking backchat directly, like hey get back my previous session
let people add their own faces and have it get its seintiment updated by AI tools
Allow for several background processes to process the chat in different ways, like depth of context, summarizers that store a compressed version as we go, private data checker
image upload interpreter
image generator using stateboard for continuity
Connect up your linkedin in two way mode
Machines with no auth being used to allow other machines to be merged with the actor
map emotional status of all affected by a proposed change
platform growth being available in chat
run help in branch with isolation
outputs of functions need to be reinstated
data permanence, with live exports
make a cli so we can send functions directly to any branch
billing limits per actor, including anonymous actors
let users send us messages using HAL
let users create issues in github for us to work on and give updates on ?
geocoding of a customer and guide user thru manual corrections plus precise location tuning
attachments sniffing using https://www.npmjs.com/package/file-type
stripe integration
pull down splices from multiple branches in ui
fix voice input in iphone
typing with a regular key or shift will update the value with the key, but not change focus
cross device testing
convert retrieving files to a GET request with optional diff info.
use the latest as a param to GET and restore the cached version on page load.
making a vector search is a special kind of tool call
make commit be a link to a viewer in a meta chat about it, to allow exploring diffs
copy button on each message
publish bots in a store
regen button on assistant or tool calls
edit button on Dave
make content sticky scroll up if we are scrolled to the bottom

...
convert to use the o1 model - if agent has no sysprompt and no tools, then no need for system instructions

start a new remote thread that only allows the single o1 agent.  switchboard would be limited to the selected agents.

While generating, write out the system prompt to the thread, or at least write out the blank reponse
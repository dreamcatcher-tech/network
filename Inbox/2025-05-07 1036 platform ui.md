...
The main app window should treat the stateboard like a browser and record all the navigation actions that occurred.  This means the agent can drive, or you can drive too. Our widgets are just those that are aware they are being driven, but any page can be put on this anvil.
...
All the workbench is doing is giving widgets to the core actions that the provider makes to the backend.

What does it mean to add the context of the entire app currently? In the context manager, I should be able to indicate that I want to just snapshot everything rather than some subpart. This would be used for solving UI bugs. 

agent driving the workbench - it can use the search functions and manipulate the context settings as well as set the current scope, which will alter what the ui is displaying.  It could also talk to a specific napp, like the web browser or the filebrowser app, and get it to do something, like load a specific page.  It could also take screenshots of the app to make sure it was doing the right thing.

If you select a chat message, then you could get a button that would open a widget that would analyze the chat message deeper. For example, it would let you see the token burn, the cost, the metadata details, the raw response. Basically everything if you wanted it. And then if that was selected it would be in the context selector so you could talk about the deeper information in the raw response. You could optionally include the system message because this should be visible too when you're in the realm of the analyzer.

To access this for the system prompt, the precursors to the current chat, we should have a sort of an initialization icon at the top which you can click on and get the stuff that happened before the actual chat occurred (where it basically says "here's maybe some settings, here's some info, this is what's being used"). All that kind of stuff, so you can basically drill into any part of the system in the lens of it being the specific data that the system knows more about. You could have just got the raw files that the chat responses stored, but using this tool means it's interpreted a certain way and you can use some of the edit tools or the helping tools to work with it. 

napps is the same as files, and you can browse the ones you have and the widgets they offer.

be able to split and merge actor accounts.

analyzing an agent just loads the workbench ui in the stateboard.

the agentic inbox.
system messaging and system processes.
system actions between other repos.
receiving messages that you do not have a linked repo for.

filesystem triggers should be a single chat thread that keeps getting updated.

You should be able to manage the permissions that the Dreamcatcher page has asked for in your root and in any widgets that want something. You grant it to them on a case-by-case basis. 

napps installs is so you can interact with napps and widgets, as well as pin to home.  napps can be transient, and you can run it without installing.  Installing means you manage the filepath its installed on, what its triggers are, what its broader permissions are.

process management needs to be visible too.

billing, and usage tracing.

wallet needs to be part of actor home.

agentic messaging.

wallets is a napp that is reached from the accounts page.  This has wigets for all major chains.

add actions / napp invokes into the chat, so can be seen what the user did.  user can delete these at any point.

add triggers to agents:
1. fs changes
2. agentic inbox messages
3. napp action
4. cron

All of the tools that we're making in the workbench could easily be represented as napps. So as long as you could navigate to the napp launcher, you could access any of the other tools possibly. 

repo permissions search for persons to invite.

repo widget needs search for repos.

notifications of invites ?

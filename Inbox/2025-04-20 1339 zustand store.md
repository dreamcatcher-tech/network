...
`useWatcher()` and then return a hook that exposes just the watched part of state.

watching a single file would be just a watcher that points at a single file ?

sync progress can be tied to each individual watch, so it makes sense from the point of view of the viewer.